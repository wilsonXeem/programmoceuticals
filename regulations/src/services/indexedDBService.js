...
class IndexedDBService {
  ...
  async storeFileBlob(path, file) {
    ...
    const { blob: compressedBlob, compressed } = await this.compressBlob(file);
  ...
  async cacheDossier(dossier) {
    ...
    await this.promisifyRequest(transaction.objectStore('dossiers').put({
      id: dossier.name || 'cached',
      name: dossier.name,
      root: dossier.root,
      createdAt: new Date().toISOString()
    }));
