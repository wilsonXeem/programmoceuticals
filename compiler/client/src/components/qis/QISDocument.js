...
function QISDocument({ onSaveToCompiler = null, savingToCompiler = false }) {
  ...
  const buildWordFileFromCurrentDocument = () => {
    ...
    const file = new File([blob], 'QIS_Document.doc', {
  ...
  const handleSaveToCompiler = async () => {
    ...
    const result = buildWordFileFromCurrentDocument();
    if (!result) {
      alert('Please ensure both front page and form are loaded before saving.');
      return;
    }
    ...
    <div style={styles.container}>
      ...
      <div className="screen-only" style={styles.navigation}>
        ...
        {typeof onSaveToCompiler === "function" && (
          ...
          >
            {savingToCompiler ? "Saving..." : "Save to Compiler (1.4.2)"}
          </button>
        )}
      </div>
